...
```{r}
mod_TRBallG_pred <- predict(mod_TRBallG, data = testingB)
```
...
NOTE: There is a rather large difference between the training RMSE (`r min(mod_TRBall$results$RMSE)`) and the testing RMSE (`r RMSE(pred = mod_TRBallG_pred, obs =testingB$medv)`).  What does this suggest? **Overfit**
...
```{r}
RF_pred <- predict(mod_RF, data = testingB)
```
...
19. Use the `train()` function with `method = "rf"`, `tuneLength = 10` along with the `myControl` as the `trControl` to fit a random forest named `mod_RF2`. Use `set.seed(42)` for reproducibility. 
...
```{r}
mod_KNN_pred <- predict(mod_KNN, data = testingB)
